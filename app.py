...
# ESTILO CSS PARA FORZAR LA CUADRÍCULA EN MÓVILES
st.markdown("""
    <style>
    .calendario-tabla { width: 100%; border-collapse: collapse; table-layout: fixed; }
    ...
    .dia-especial { background-color: #fff4e6; border-left: 3px solid #d4af37 !important; font-weight: bold; }
...
for i, tab in enumerate(tabs):
    with tab:
        ...
        # --- CALENDARIO EN FORMATO TABLA (Móvil Friendly) ---
        cal = calendar.monthcalendar(2026, mes_num)
        
        html_cal = '<table class="calendario-tabla"><tr>'
        ...
        for semana in cal:
            ...
            for dia in semana:
                if dia == 0:
                    ...
                    html_cal += f'<td class="{clase}">{dia}<br><span style="font-size:8px;">{emoji}</span></td>'
            html_cal += '</tr>'
        html_cal += '</table>'
